...
func _ready() -> void:
	...
	music_player.change_instrument(11)
	music_player.play_note(57) # Nice
	
	await get_tree().create_timer(1).timeout
	music_player.change_instrument(9)
	music_player.play_note(76) # Nice
	
	#
	#music_player.change_instrument(10)
	#
	#music_player.play_note(67) 
